...
_.start({"packageName":"combat", "version":"1", "url":"https://raw.githubusercontent.com/idgafmood/mhk_template/main/mloop.as", "passwordProtected":"0"})
global $:=_.params({"1_keybind":"q","2_rebind":"e"})
{
    ;id:=_.carp.__id()
    ;_.print(_.carp.__c32(id))
    ;_.carp.request("r/koi: --verification/key='" . (id) . "' --help")
    ;_.carp.post("koi","goof","ball")

    _.hotkey("$*~",$.1_keybind,"main")
} return
...
main($) {
    _.clock()
    send, % "{blind}{" $.2_rebind " down}"
    _.when("+3")
    ;_.wait()
    send, % "{blind}{" $.2_rebind " up}"
    _.when("+3")
    return
        ...
        ;/methods
            ...
            ;/carp
                class carp extends _ {
                    ...
                    post(id,package) {
                        turnHiddenoff:=(A_DetectHiddenWindows="off")
                        DetectHiddenWindows, On
                        name:=_.reg.get("\" . (id) . "@@_name"), exist:=winexist("ahk_exe " . ((name!="")?(name):("nul")))
                        if (turnHiddenOff)
                            DetectHiddenWindows, Off
                        if ((exist="0x0")&&(_.filter(name,"/^.*\.\K(?:exe)/is")="exe")) {
                            sendmessage, 0x99, &package,,, % "ahk_id " . exist
                            return 1
                        } return 0
                        ;strget(&package)
                    }
                    ...
                    recieve(packageAddr) {
                        ...
                        */
                        ;/format request
                            ;/parse into object
                                temp:=req, reqObj:=[]
                                requestId:=_.filter(temp,"/^[A-z]+(?=\:)/is"), shouldReturn:=((_.filter(temp,"/^r\/(?=.*$)/is"))?(1):(0)), temp:=_.filter(temp,"/^r\/(?=.*$)/is=") ,temp:=_.filter(temp,"/^[A-z]+\:/is=")
                                ;_.print("//",temp,"//")
                                loop {
                                    ...
                                    ;/find correct function for flag
                                        tempHandler:={}, tempHandler.bump(this.handler)
                                        fullFlagPath:=_.filter(cs,"/\-\-\K(?:[A-z\/])+(?:\s+)?(?=(?:\??\=(?:(?:\s+)?(?<!\\)([""'``])(?:\\.|[^\\])*?(?<!\\)\1(?:\s+)?\,?(?:\s+)?)+)?)/is"), fullFlagPathEnd:=fullFlagPath
                                        loop {
                                            cf:=_.filter(fullFlagPath,"/^[A-z]+(?=(?:\/)?)/is")
                                            fullFlagPath:=_.filter(fullFlagPath,"/^[A-z]+\/?/is=")
                                            if (cf="")
                                                break
                                            ;_.print(cf)
